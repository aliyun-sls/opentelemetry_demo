...
spec:
  ...
  template:
    ...
    spec:
      ...
      {{- end }}
      serviceAccountName: {{ .serviceAccountName | default (include "otel-demo.serviceAccountName" .) }}
      ...
      initContainers:
          - name: {{ .initContainers.name }}
            image: {{ .defaultValues.initContainers }}
            command: {{ .initContainers.command | toYaml | nindent 14 }}
            {{- if .initContainers.volumeMounts }}
            volumeMounts: {{ .initContainers.volumeMounts | toYaml | nindent 14 }}
            {{- end }}
      {{- end}}
      volumes:
        {{- range .mountedConfigMaps }}
